...
const InlineLogin: React.FC<{onLogin: (name?: string, oab?: string) => void}> = ({ onLogin }) => {
  ...
  const handleAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === "login") {
        ...
        // 2. Otherwise look in Firestore real-time cloud database
        ...
        if (userInFirestore) {
          ...
          // 3. Fallback to local storage for offline support and transition
          ...
          if (matchedLocalUser) {
            ...
            setErr("Este e-mail não foi cadastrado no Banco de Dados em Nuvem.");
          }
        }
      } else {
        // Registration flow
        ...
        if (savedToCloud) {
          ...
          setSuccess("Usuário cadastrado localmente (banco offline)! Conectando...");
        }
  ...
  const fillDefaultCredentials = () => {
    setEmail("gustavodantas.advogados@gmail.com");
    setPassword("GD2026");
    setAuthMode("login");
    setErr("");
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="bg-[#0b101f] border border-[#1e293b]/40 rounded-2xl p-8 max-w-md w-full shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] text-center space-y-7 relative overflow-hidden">
        {/* Glow decoration */}
        ...
        <div className="space-y-2.5 relative z-10">
          <div className="mx-auto w-14 h-14 rounded-2xl bg-indigo-500/10 border border-indigo-500/25 flex items-center justify-center text-indigo-400 shadow-inner">
            <Scale className="w-7 h-7" />
          </div>
          <h1 className="text-2xl font-bold text-white tracking-tight font-display">JurisFinance <span className="text-[#8b5cf6]">AI</span></h1>
          <p className="text-xs text-slate-400 font-medium">Controle de Advocacia e Finanças Integradas</p>
        </div>

        {/* Tab switcher */}
        <div className="flex bg-[#070b16] p-1 rounded-xl border border-slate-800/80 relative z-10">
          <button
            type="button"
            onClick={() => { setAuthMode("login"); setErr(""); setSuccess(""); }}
            className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all uppercase tracking-wider cursor-pointer ${
              authMode === "login" 
          ...
          <button
            type="button"
            onClick={() => { setAuthMode("register"); setErr(""); setSuccess(""); }}
            className={`flex-1 py-2 rounded-lg text-[10px] font-bold transition-all uppercase tracking-wider cursor-pointer ${
              authMode === "register" 
                ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10" 
                : "text-slate-400 hover:text-slate-200"
        ...
        <form onSubmit={handleAuthSubmit} className="space-y-4 text-left relative z-10" id="auth-form">
          {authMode === "register" && (
            <>
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider font-mono">Nome Completo</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[#050811] border border-slate-800/80 rounded-xl py-2 pl-10 pr-3 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-indigo-500 font-medium transition-colors disabled:opacity-50"
                  />
                </div>
              </div>

              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider font-mono">Número OAB (Opcional)</label>
                <div className="relative">
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider font-mono">E-mail</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="exemplo@oab.com.br"
                className="w-full bg-[#050811] border border-slate-800/80 rounded-xl py-2 pl-10 pr-3 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-indigo-500 font-medium transition-colors disabled:opacity-50"
              />
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider font-mono">Senha</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder={authMode === "login" ? "Senha de acesso" : "Crie uma senha"}
                className="w-full bg-[#050811] border border-slate-800/80 rounded-xl py-2 pl-10 pr-10 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-indigo-500 font-mono transition-colors disabled:opacity-50"
              />
              <button
                type="button"
          ...
          {authMode === "register" && (
            <div className="space-y-1">
              <label className="text-[10px] uppercase font-bold text-slate-400 tracking-wider font-mono">Confirmar Senha</label>
              <div className="relative">
                <Lock className="absolute left-3 top-2.5 w-4 h-4 text-slate-500" />
                <input
                  type={showPassword ? "text" : "password"}
                  ...
                  className="w-full bg-[#050811] border border-slate-800/80 rounded-xl py-2 pl-10 pr-3 text-xs text-white placeholder-slate-500 focus:outline-hidden focus:border-indigo-500 font-mono transition-colors disabled:opacity-50"
                />
              </div>
            </div>
          )}

          {err && <p className="text-[11px] text-rose-500 font-semibold text-center mt-2 p-2 bg-rose-500/10 rounded-lg border border-rose-500/20">{err}</p>}
          {success && <p className="text-[11px] text-emerald-500 font-semibold text-center mt-2 p-2 bg-emerald-500/10 rounded-lg border border-emerald-500/20">{success}</p>}

          <button
            type="submit"
            disabled={loading}
            className="w-full mt-4 py-3 bg-indigo-600 hover:bg-indigo-500 active:bg-indigo-700 text-white font-bold text-xs rounded-xl transition-all shadow-md cursor-pointer hover:shadow-indigo-600/10 uppercase tracking-widest flex items-center justify-center gap-2 disabled:bg-slate-700 disabled:cursor-not-allowed"
          >
            {loading ? (
              <span className="flex items-center gap-1.5">
                <span className="animate-spin text-indigo-300">⏳</span> Conectando com a Nuvem...
              </span>
            ) : authMode === "login" ? (
              <>
                <LogIn className="w-4 h-4" />
                Acessar Plataforma Premium
              </>
            ) : (
              <>
        ...
        {authMode === "login" && (
          <div className="bg-[#050811] p-3.5 rounded-xl border border-[#1e293b]/40 text-left space-y-2 relative z-10 shadow-inner">
            <div className="flex items-center justify-between border-b border-slate-800/50 pb-1.5">
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider font-mono">Demonstração Predefinida:</span>
              <button
                type="button"
                onClick={fillDefaultCredentials}
                className="text-[9px] font-bold text-indigo-400 hover:text-indigo-300 uppercase tracking-wider underline cursor-pointer"
              >
                Auto-Preencher
              </button>
            </div>
            <div className="text-[11px] space-y-1 font-mono text-slate-300 bg-[#070b16]/60 p-2.5 rounded-lg border border-slate-800/50 leading-tight">
              <p><span className="text-slate-500">Email:</span> gustavodantas.advogados@gmail.com</p>
              <p><span className="text-slate-500">Senha:</span> GD2026</p>
            </div>
          </div>
        )}

        <div className="text-[10px] text-slate-400 bg-[#050811]/60 p-2.5 rounded-xl border border-[#1e293b]/20 leading-normal text-left relative z-10">
          💡 <b>Segurança Local:</b> Os dados de login são armazenados de forma 100% privada no seu navegador. Caso troque de endereço (URL) ou use outro dispositivo/computador, basta clicar em <b>Criar Conta</b> e registrar o nome e senha desejados naquele novo endereço para ter acesso instantâneo!
        </div>
      </div>
    </div>
  );
